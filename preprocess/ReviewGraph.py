...
class ReviewGraphBuilder:
    def __init__(self,
        ...
        self.review_A= self.getReview(A_user_review_dict, A_item_user_dict)
        self.get_tfidf_edge()
        self.get_pmi_edge()
    ...
    def get_pmi_edge(self):
        ...
        for edge_item in pmi_edge_lst:
            ...
            self.gA.add_edge(word_indx2, word_indx1, weight=edge_item[2])           

        print_graph_detail(self.gA)

    def get_tfidf_edge(self):
        # 获得tfidf权重矩阵（sparse）和单词列表
        tfidf_vec_A = self.get_tfidf_vec()
